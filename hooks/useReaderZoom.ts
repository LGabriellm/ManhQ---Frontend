...
export function useReaderZoom(options: UseReaderZoomOptions = {}) {
  ...
  const handleTouchMove = useCallback(
    (e: TouchEvent) => {
      ...
      if (e.touches.length === 2 && lastPinchDist.current !== null) {
        ...
        setZoom((prev) => {
          const newScale = Math.max(
            minScale,
            Math.min(maxScale, prev.scale * ratio),
          );
          if (newScale <= 1.05)
    ...
    [minScale, maxScale, clampPan, zoom.scale],
  );
  ...
  const handleTouchEnd = useCallback(
      ...
      if (e.changedTouches.length === 1 && e.touches.length === 0) {
        ...
        if (
          ...
          if (isZoomed) {
            ...
            const tapRelX =
              (tap.x - rect.left - rect.width / 2) / rect.width;
            const tapRelY =
              (tap.y - rect.top - rect.height / 2) / rect.height;
            const clamped = clampPan(
              (-tapRelX * rect.width * (doubleTapScale - 1)) / doubleTapScale,
              (-tapRelY * rect.height * (doubleTapScale - 1)) / doubleTapScale,
              doubleTapScale,
              rect,
            );
            setZoom({
    ...
    [isZoomed, doubleTapScale, clampPan],
  );
