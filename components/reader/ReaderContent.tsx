...
export function ReaderContent({
  ...
  const handleClose = useCallback(() => {
    if (isOfflineMode && onClose) {
      onClose();
    } else {
      router.push(`/serie/${seriesId}`);
    }
  }, [isOfflineMode, onClose, router, seriesId]);

  const handleNavigateChapter = useCallback((targetChapterId: string) => {
    if (isOfflineMode && onNavigateChapter) {
      onNavigateChapter(targetChapterId);
    } else {
      router.push(`/reader/${seriesId}/${targetChapterId}`);
    }
  }, [isOfflineMode, onNavigateChapter, router, seriesId]);
